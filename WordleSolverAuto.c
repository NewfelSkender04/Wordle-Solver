...
char **read_words(const char *filename, int *count) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strlen(line) != WORD_LEN) continue;  // Ignore les mots de mauvaise longueur

        // Réallocation dynamique du tableau
        words = realloc(words, (n + 1) * sizeof(char *));
        words[n] = malloc(WORD_LEN + 1);
        for (i = 0; i < WORD_LEN + 1; i++) words[n][i] = 0;  // Initialisation à 0
        strcpy(words[n], line);
        n++;
    }
...
void compute_feedback(const char *guess, const char *target, char fb[WORD_LEN + 1]) {
    ...
    for (i = 0; i < WORD_LEN; i++) {
        if (fb[i] == 'g') continue;  // On ignore les lettres déjà vertes
...
int keep_word(const char *word, const char *guess, const char *fb) {
    ...
    for (i = 0; i < WORD_LEN; i++) used[i] = 0;

    /* Vérification des lettres vertes */
    for (i = 0; i < WORD_LEN; i++) {
        if (fb[i] == 'g') {
            if (word[i] != guess[i]) return 0;  // Doit avoir la même lettre à la même position
    ...
    for (i = 0; i < WORD_LEN; i++) {
        if (fb[i] == 'y') {
            if (word[i] == guess[i]) return 0;  // Ne doit pas être à la même position
            int found = 0;
            ...
            if (!found) return 0;  // La lettre doit être présente ailleurs
        }
    }

    /* Vérification des lettres noires */
    for (i = 0; i < WORD_LEN; i++) {
        if (fb[i] == 'b') {// La lettre ne doit pas apparaître dans les positions non utilisées
            for (j = 0; j < WORD_LEN; j++) {
                if (!used[j] && word[j] == guess[i]) return 0;
            }
        }
    }
...
void print_feedback(const char *guess, const char *fb) {
    int i;
    for (i = 0; i < WORD_LEN; i++) {
        if (fb[i] == 'g') printf("\x1b[42m %c \x1b[0m", guess[i]);  // Fond vert
        else if (fb[i] == 'y') printf("\x1b[43m %c \x1b[0m", guess[i]);  // Fond jaune
        else printf("\x1b[40m %c \x1b[0m", guess[i]);  // Fond noir
    }
    printf("\n");
}
...
int main() {
    ...
    for (attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        for (i = 0; i <= attempt; i++) print_feedback(grid[i], fb_grid[i]);
        printf("\n");

        if (strcmp(fb, "ggggg") == 0) break;  // Mot trouvé !
